...
f = open("latest_quake.txt", mode="w")
f.write("")
f.close
...
class MainWindow(QMainWindow):
    ...
    def scan_quakes(self):
        ...
        for i in range(len(latest_five_list)):
            ...
            f = open("latest_quake.txt", mode="a")
            f.write("*"+latest_five_list[i].getText() + "*" + "\n")
            f.close()
        ...
        if latest_time != text_formatted[-1]:
            os.system('start deneme.mp3')
            latest_time = text_formatted[-1]
        else:
            pass
        print(f"LATEST TIME is {latest_time}")
        print(f"New Quake's TIME is {text_formatted[-1]}")
        



def module_scan_quakes():
...
def thread_scan():
    t1 = Thread(target=module_scan_quakes)
    t1.setDaemon(True)
    t1.start()
